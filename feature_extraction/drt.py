...
def sequential_batch_disk_radon_features(human_protein_atlas, ids, save_dir, batch_size=10, training=True):
    for i, id_subset in enumerate(np.array_split(ids, batch_size)):
        filename = os.path.join(save_dir,
                                "radon_features_" + str(i) + ("_train" if training else "_test") + ".radon_data")
...
def multiprocessed_batch_disk_radon_features(human_protein_atlas, ids, save_dir, batch_size=30, max_processes=2,
                                             training=True):
    # Get CPU count
    try:
    ...
    for i, id_subset in enumerate(np.array_split(ids, batch_size)):
        ...
        if len(processes) % min(n_cpus,
                                max_processes) == 0:  # wait for oldest process to finish before spawning a new one
            processes[0].join()
            processes.pop(0)
