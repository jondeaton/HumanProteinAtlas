...
def get_features(image, method=Feature.drt, grayscale=True, binary=False):
    # Preprocess
    if grayscale:
...
"""
def get_local_binary_patterns(image, neighbors=None, radius=3):
    numPoints = neighbors if neighbors != None else 8 * radius
...
def get_orb_features_2(image, n_keypoints=5, patch_size=20):
    features = []

    key_points= get_orb_keypoints(image, n_keypoints)

    if len(key_points) != n_keypoints:
        print("Error", len(key_points))

        if len(key_points) == 0:
            key_points = np.array([256, 256] * n_keypoints)
        else:
            key_points = np.concatenate((key_points, [key_points[0]] * (n_keypoints - len(key_points))))
...
def get_hog_features(image):
    ...
    features, hog_image = hog(reshaped_image, orientations=10, pixels_per_cell=(16, 16), cells_per_block=(1, 1), multichannel=True, visualize=True)
